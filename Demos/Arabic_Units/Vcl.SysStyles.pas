...
function TSysPopupStyleHook.GetDevicePPI: Integer;
begin
...
begin
  ...
  CaptionHeight := ElementSize.Height + FStretchedCaptionInc;
...
begin
  ...
  begin
    ...
    if not StyleServices.GetElementContentRect(0, LDetails, CaptionRect, Result) then
      Result := Rect(0, 0, 0, 0);
    if (FStretchedCaptionInc > 0) and (Result.Height > 0) then
      OffsetRect(Result, 0, -Result.Top + (CaptionRect.Height - Result.Height) div 2);
  end;
end;
...
function TSysDialogStyleHook.GetDevicePPI: Integer;
begin
  if CheckPerMonitorV2SupportForWindow(Handle) then
    Result := GetDPIForWindow(Handle)
  else
...
begin
  ...
  if PPI > 96 then
  begin
    Result.Top := MulDiv(Result.Top, PPI, 96);
...
var
  ...
  DC: HDC;
  FButtonState: TThemedWindow;
  LCaptionRect, LBorderSize, R, R2: TRect;
...
begin
  ...
  begin
    ...
    StyleServices.DrawText(CaptionBmp.Canvas.Handle, CaptionDetails, LText, TextRect, TextFormat, clNone, PPI);
    MoveWindowOrg(CaptionBmp.Canvas.Handle, 0, -TextTopOffset);
  end
  else
  begin
