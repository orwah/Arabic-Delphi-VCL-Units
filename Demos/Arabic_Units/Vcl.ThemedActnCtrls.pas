...
GlyphLayout:= blGlyphright ;
  ...
  try
    ...
    try
      ...
      // Check/Glyph
      StyleServices.GetElementSize(DC, StyleServices.GetElementDetails(CheckMarkStates[Enabled]), esActual, LGlyphSize, PPI);
      StyleServices.GetElementMargins(DC, StyleServices.GetElementDetails(CheckMarkStates[Enabled]), emContent, LCheckMargins, PPI);
      ...
      // Menu item
      StyleServices.GetElementMargins(DC, StyleServices.GetElementDetails(tmPopupItemNormal), emSizing, LMenuItemMargins, PPI);
      StyleServices.GetElementSize(DC, StyleServices.GetElementDetails(SubMenuStates[Enabled]), esActual, LSubMenuGlyphSize, PPI);
...
begin
  ...
  begin
    ...
    begin
      LPPI := GetDevicePPI;
      StyleServices.GetElementSize(Canvas.Handle, StyleServices.GetElementDetails(CheckMarkStates[Enabled]), esActual, LCheckSize, LPPI);
      ...
      StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(CheckMarkStates[Enabled]), R, nil, LPPI);
    end;
  end;
end;
...
begin
 ...
 OffsetRect(LRect, -LRect.Left + 4, 1)
  ...
  if StyleServices.GetElementSize(Canvas.Handle,
    StyleServices.GetElementDetails(tmPopupSubMenuNormal), esActual, LSize, PPI)
  then
  begin
 ...
 StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(SubMenuStates[Enabled]), LRect, nil, PPI);
end;
...
begin
  ...
  try
    ...
    try
      ...
      StyleServices.GetElementSize(DC, StyleServices.GetElementDetails(SubMenuStates[Enabled]), esActual, LSubMenuGlyphSize, PPI);
      StyleServices.GetElementMargins(DC, StyleServices.GetElementDetails(SubMenuStates[Enabled]), emContent, LSubMenuGlyphMargins, PPI);
...
begin
  ...
  begin
    ...
    begin
      StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(CheckMarkBkgStates[Enabled]), FCheckRect);
      StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(CheckMarkStates[Enabled]), FCheckRect, nil, PPI);
    end;
  end
  else
  begin
  ...
  begin
    ...
    StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(CheckMarkStates[Enabled]), FVisibleCheckRect, nil, PPI);
  end;
end;
