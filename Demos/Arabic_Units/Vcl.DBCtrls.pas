...
procedure TDBComboBox.WndProc(var Message: TMessage);
  ...
  begin
    ...
    if Enabled then
      Details := StyleServices.GetElementDetails(tcBorderNormal)
    else
      Details := StyleServices.GetElementDetails(tcBorderDisabled);
    ...
    StyleServices.DrawElement(DC, Details, R, nil, PPI);
    if AText <> '' then
    begin
      ItemR := ClientRect;
...
begin
  ...
  begin
    ...
    if Byte(Accept) <> 2 then SetFocus;
    ...
    Invalidate;
    SearchText := '';
    if Accept and (Byte(Accept) <> 2) and CanModify then SelectKeyValue(ListValue);
...
begin
  ...
  begin
    ...
    StyleServices.DrawElement(Canvas.Handle, Details, R, nil, PPI);
  end
  else
  begin
    if not (ListActive and Enabled) then
