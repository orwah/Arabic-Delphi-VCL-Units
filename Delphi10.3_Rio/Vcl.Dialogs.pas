...
//orwah
  type
  MyExceptionClass = class sealed
    class procedure MyExceptionHandler(Sender: TObject; E: Exception);
  end;
...
var
  ...
  B,b1,b2, CancelButton: TMsgDlgBtn; // orwah
{$IF DEFINED(CLR)}
  IconID: Integer;
{$ELSE}
  IconID: PChar;
...
begin
  ...
  begin
    Font.Assign(Screen.MessageFont);
    Font.Height := Muldiv(Font.Height, Result.CurrentPPI, Screen.PixelsPerInch);
...
function InternalDoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType;
  ...
  AlternateTaskDlgFlags: TTaskDialogFlags): Integer; forward;
...
function InternalDoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string; DefaultButton: TMsgDlgBtn;
  UseAlternateTaskDlgFlags: Boolean; AlternateTaskDlgFlags: TTaskDialogFlags): Integer;
const
...
//orwah
class procedure MyExceptionClass.MyExceptionHandler(Sender: TObject; E: Exception);
begin

if E is EDivByZero then
MessageDlg( 'حدث الخطأ التالي : ' +#13+#13+ 'لايمكن القسمة على صفر' +#13,mtError,[mbOK],0)
//else  if E is EConvertError  then
//        MessageDlg( 'حدث الخطأ التالي : ' +#13+#13+ 'قيمة غير صحيحة!' +#13+E.Message,mtError,[mbOK],0)
else     if Pos('duplicate values', E.Message) <> 0 then
     MessageDlg('حدث الخطأ التالي : ' +#13+'لايمكن الحفظ , البيانات مكررة !'+#13+ E.Message+#13,mtError,[mbOK],0)
else
 MessageDlg( 'حدث الخطأ التالي : ' +#13+#13+ E.Message+#13,mtError,[mbOK],0);
